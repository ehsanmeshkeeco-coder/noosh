...
interface HealthAlertEventDao {
    ...
    @Query("UPDATE health_alert_events SET deliveryStatus = 'SENT', sentAt = :sentAt WHERE eventId = :eventId")
    ...
    @Query("UPDATE health_alert_events SET deliveryStatus = 'FAILED', retryCount = retryCount + 1, errorMessage = :error WHERE eventId = :eventId")
